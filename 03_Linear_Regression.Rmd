...
RSS, beta_1, beta_2, Contour of RSS, Population regression line, least square line, 

SE of beta, CI of beta, Hypothesis testing of beta, t-statistic of beta, 

RSE, R square, Cor, F-statistic

Qualitative var and dummy var, 

Interaction term, additive assumption, 

non-linearity, correlation of error terms, 

Heteroscedasticity, 

outliers, leverage point, influential point, 
...
predict(lm.fit, data.frame((lstat = c(5, 10, 15))), 
        interval = "prediction")
```
### Why prediction interval wider than confidence interval. 

__Confidence interval__: 

- Uncertainty in the estimation of population mean--population parameter

$$
\bar{y} \pm t_{\alpha/2, n -1} \cdot\frac{s}{\sqrt{n}}
$$

Confidence interval is the interval for the mean. 

__Prediction Interval__:

$$
\bar{y} \pm t_{\alpha/2, n -1} \cdot s \sqrt{1 + \frac{1}{n}}
$$

Prediction interval is wider than confidence because of two sources of uncertainties in prediction: 

the uncertainties of the mean, i.e., regression line, and 

the uncertainties of response, i.e., the $\epsilon$
...
### How to interpret the QQ plot of regression diagnosis. 

Normal error assumption, Ideally, the residuals will be very close to normal distribution and fall around the 45 degree line. 

Heavy Tails: If points diverge from the line at both ends but are close in the middle, the residuals have more extreme values than expected (heavy-tailed distribution).

Light Tails: If points are close to the line at the ends but diverge in the middle, the residuals have fewer extreme values than expected (light-tailed distribution).

Positive Skewness: If points curve below the line on the left and above the line on the right, the distribution is right-skewed.

Negative Skewness: If points curve above the line on the left and below the line on the right, the distribution is left-skewed.
...
### Hatvalue and the empirical threshold. 
...
## Lab3: Interaction Terms

```{r}
summary(lm(medv ~ lstat * age, data = Boston))
```

## Lab4: Non-Linear Transformation of the Predictors. 
...
### 3E1: the hypothesis testing of the coef. null hypothesis, beta = 0, alternative, beta != 0. 

### 3E2: KNN in classification and regression. 
...
$$
\hat{f}(x_0)=\arg\max_{j} Pr(Y = j|x = x_0)
$$
...
### 3E3:Application of multiple linear regression and interpretation of the coef, inc interaction. 

Note: the effect of coef is not related to the size of the number, use t test and it depends on the value. 

### 3E4: Model flexibility, training errors and testing errors. 

Cubic model is more flexible than linear model. training error of cubic smaller than linear model. testing error of cubic 

### 3E5: Linear regression through the origin

### 3E6: In the case of simple linear regression, least square line always pass through the mean point. 

### 3E7: The R square statistic is equal to the square of the correlation between X and Y. Lot's of algebra to proof. 

### 3E8: Horsepower univariate, RSE, ggfortify
...
```{r}
predict(mpg_horsepower, 
        data.frame(horsepower = 98), 
        interval = "confidence", level = 0.95)
```

```{r}
predict(mpg_horsepower, data.frame(horsepower = 98), 
...
ggplot(Auto, aes(x = horsepower, y = mpg)) + 
  geom_point() + 
  geom_abline(intercept = coef(mpg_horsepower)[1], 
              slope = coef(mpg_horsepower)[2], 
              col = "deepskyblue3", 
              size = 1) + 
  geom_smooth(se = F)
...
autoplot(mpg_horsepower)
```

### 3E9: Feature plot, full dressed MLR, cook'distance

a) Feature plot of variables. 
...
b) COrrelation between variables. see above
...
summary(mpg_lm)
```
c-1: Is there a relationship between the predictors and the response. 

F-statistic. 

c-2: Find predictors of statistic significance. 

c-3: Intrepret of the slope, holding other predictor constant. 
